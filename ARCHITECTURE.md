...
| `stepbuilder/pads.py` | 370 | 16 | the copper pads (round 85): `pin_sides` (which outer face(s) of the pin's zone its span reaches, and which pad - a surface padstack's one pad, a through padstack's by layer, backwards when mirrored), `pad_face` (the outline as a face at the origin, the donut's hole, the drill cut out, mirrored x -> -x; `_settle_offset` checks bbox + offset against the outline - the outline already stands at the offset, measured on the Dell board), `pad_solid` (the face extruded `PAD_THICKNESS` away from the board), `_placement` (turn, move, fold), `_Zones` (point -> outer conductors + faces), `build_pads` (one shared thin solid per (padstack, layer, mirrored, side), instanced per pin under `pads_top` / `pads_bot`), `PadsResult` |
...
flowchart TD
    ...
    subgraph PY["stepbuilder — the window and the build"]
        ...
        S --> S2["copper pads, when ticked: pin_sides by span against the zone's outer copper,<br/>one thin solid per figure, instanced per pin × fold.transform_at"]
