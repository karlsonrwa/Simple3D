...
(`Scripts\stepbuilder-py\stepbuilder\`). It must be exactly
`Scripts\stepbuilder\__main__.py`. Verify from a `cmd`:

```
cd /d d:\Projects\OrCAD\Scripts
...
```
load("d:/Projects/OrCAD/Scripts/makeVariant3dIntermediates.il")
load("d:/Projects/OrCAD/Scripts/simple3d.il")
```
...
|---|---|
| `S3D_ScriptDir` | Folder holding the `stepbuilder` package. Used to launch Python without depending on the current directory. |
...
| `S3D_DefaultModelDir` | Fallback folder for 3D models, used only if the `cad` folder cannot be resolved. |
...
Flags: `--z-datum {top,bottom}`, `--color NAME|r,g,b|#rrggbb`, `--rim-color ...`,
`--dated-name`, `--brd-name NAME`, `--mfr-pn-in-name`, `--no-minimize`,
`--legacy-color`. Exit code 0 on success, 1 on error.
...
(`Scripts\stepbuilder-py\stepbuilder\`). Должно быть ровно
`Scripts\stepbuilder\__main__.py`. Проверьте из `cmd`:
...
|---|---|
| `S3D_ScriptDir` | Папка с пакетом `stepbuilder`. Нужна, чтобы запускать Python независимо от текущего каталога. |
...
| `S3D_DefaultModelDir` | Запасная папка для 3D-моделей, используется только если не удалось определить папку `cad`. |
...
Флаги: `--z-datum {top,bottom}`, `--color ИМЯ|r,g,b|#rrggbb`, `--rim-color ...`,
`--dated-name`, `--brd-name ИМЯ`, `--mfr-pn-in-name`, `--no-minimize`,
`--legacy-color`. Код возврата 0 при успехе, 1 при ошибке.
