...
| **Flat** | Draw the legend as surfaces lying on the board face instead of thin solids standing off it: about a quarter of the silkscreen's file size. See *Silkscreen file size* below. |
...
1. **Flat.** A solid costs one face per polygon edge plus a top and a bottom; a
   surface costs one face. The face lies **on** the board face, which is what a
   zero-thickness legend should mean. What you give up: the ink is a surface,
   not a solid - no thickness to measure, and nothing downstream can do boolean
   work with it. Two coplanar faces can also flicker against each other in a
   viewer that resolves depth per pixel; if that shows up, use solid mode, which
   genuinely stands off the board.
...
`--no-silk-bottom`, `--flat-silkscreen`, `--silk-color White|Black`,
`--no-minimize`, `--legacy-color`, `--quiet`. Exit code 0 on success, 1 on error.
...
| **Flat** | Рисовать легенду поверхностями, лежащими на грани платы, вместо тонких тел, стоящих на ней: примерно вчетверо меньший вклад в размер файла. См. «Размер файла и шелкография» ниже. |
...
1. **Flat.** Тело стоит по грани на каждое ребро полигона плюс верх и низ;
   поверхность — одну грань. Грань лежит **на** грани платы — именно это и должна
   означать легенда нулевой толщины. Чем платите: краска становится
   поверхностью, а не телом — толщину не измерить и булевы операции с ней
   невозможны. Две совпадающие плоскости к тому же могут мерцать друг о друга в
   просмотрщике, разрешающем глубину попиксельно; если это проявится —
   переключитесь на объёмный режим, он реально стоит над платой.
