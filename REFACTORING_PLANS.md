...
| F2 | `tests/skill_transliterations.py`: the Python copies of `s3dJsonQuote`, `s3dDrillXY`, `s3dDesignFolder`/`s3dVariantFilePath`, the variant rule, `tconc`, `s3dBendsJson`, `s3dLayerInBody`, `s3dLayerIsNegative`, `skill_merge`, `s3dSpanAcross` in one module, each with a `# mirrors makeVariant3dIntermediates.il:<procedure>` line | the SKILL side has one executable specification file — **done, round 80** for the copies that existed: 12 blocks cut verbatim out of six suites (`s3dJsonQuote` with its control-character class, `s3dAddIndent`, the writer's fragments and `makePcb`/`create3dIntermediateFormat`, `rotateXY`/`s3dDrillXY`, the folder pair, `s3dVariantFit`, `s3dSymbolsToExport`'s cond, the parser's `awaitEndCondition` branch, `s3dIsRefdesToken`, `Tconc`/`tconc`, `s3dJsonMerge`, `s3dLayerInBody`), each under a `# mirrors skill/<part>.il: <procedure>` line; the suites import what they defined, and every one passed unchanged. `s3dBendsJson`, `s3dLayerIsNegative` and `s3dSpanAcross` never had a Python copy - the fold and layer suites test the Python side's own functions - so there was nothing to move |
...
shipped v9. F2 (the transliterations in one module) is done too. Left: only
the optional F4 (pytest) - and E3, when a release has shipped v9.
