...
- **Copper pads** — the copper of every pin's pad on the outer faces, as
  copper-coloured solids 10 µm high standing on the mask. Off by default; needs
  a JSON exported with this version (`format_version` 10). Vias are not drawn
  (README, *Copper pads*).
...
- **Fold flex bends** — согнуть плату по зонам сгиба из Allegro (*Setup –
  ...
  (README, *K-фактор*).
- **Copper pads** — медь площадок всех выводов на наружных гранях, телами
  цвета меди высотой 10 мкм, стоящими на маске. По умолчанию выключено; нужен
  JSON, экспортированный этой версией (`format_version` 10). Переходные
  отверстия не рисуются (README, *Медь площадок*).
